...
ml_bool ML_FUNC ML_AABBIntersect(const ml_aabb* pBox1, const ml_aabb* pBox2, ml_aabb* pIntersect)
{
	...
	if(pIntersect)
	{
		pIntersect->v3Min.x=ML_Max(pBox1->v3Min.x, pBox2->v3Min.x);
		pIntersect->v3Max.x=ML_Min(pBox1->v3Max.x, pBox2->v3Max.x);
		pIntersect->v3Min.y=ML_Max(pBox1->v3Min.y, pBox2->v3Min.y);
		pIntersect->v3Max.y=ML_Min(pBox1->v3Max.y, pBox2->v3Max.y);
		pIntersect->v3Min.z=ML_Max(pBox1->v3Min.z, pBox2->v3Min.z);
		pIntersect->v3Max.z=ML_Min(pBox1->v3Max.z, pBox2->v3Max.z);
	}
	return ML_TRUE;
}
...
ml_aabb* ML_AABBFromVec3s(ml_aabb* pOut, const ML_VEC3* pVList, const ml_dword nNumVecs)
{
	...
	{
		pOut->v3Min.x=ML_Min(pVList[i].x, pOut->v3Min.x);
		pOut->v3Min.y=ML_Min(pVList[i].y, pOut->v3Min.y);
		pOut->v3Min.z=ML_Min(pVList[i].z, pOut->v3Min.z);
		
		pOut->v3Max.x=ML_Max(pVList[i].x, pOut->v3Max.x);
		pOut->v3Max.y=ML_Max(pVList[i].y, pOut->v3Max.y);
		pOut->v3Max.z=ML_Max(pVList[i].z, pOut->v3Max.z);
	}
