	...
	int nStart)
{
	...
	szTemp=LG_Malloc(L_strlen(szParams)+1);
	if(!szTemp)
		return LG_FALSE;
...
int Con_AddEntry(HLCONSOLE hConsole, char* szEntry)
{
	...
	lpNew=LG_Malloc(sizeof(LCENTRY));

	if(lpNew==NULL)
	{
		return 0;
	}
	lpNew->lpstrText=LG_Malloc(sizeof(char)*lpConsole->nMaxStrLen);
	...
	void* pExtraData)
{
	...
	lpConsole=LG_Malloc(sizeof(LCONSOLE));
	if(lpConsole==NULL)
	{
...
int Con_SendMessage(HLCONSOLE hConsole, char* szMessage)
{
	...
	szTemp=LG_Malloc(lpConsole->nMaxStrLen);
...
int Con_SendErrorMsg(HLCONSOLE hConsole, char* format, ...)
{
	...
	szOutput=LG_Malloc( ((LPLCONSOLE)hConsole)->nMaxStrLen );
...
int Con_SendCommand(HLCONSOLE hConsole, char* szCmdLine)
{
	...
	szParams=LG_Malloc(lpConsole->nMaxStrLen);
	szCommand=LG_Malloc(lpConsole->nMaxStrLen);
	...
	/* First attempt to process the command if the command isn't prcoessed
	   then attempt to process internal commands, if asked to if we
		process an internal command we return, if not we process the
		command function. */
	if(lpConsole->CommandFunction)
	{
		bResult=lpConsole->CommandFunction(nCommand, szParams, hConsole, lpConsole->pExtraData);
	}
	else
	{
	...
	//If the command hasn't been processed and the console is supposed
	//to attempt to process commands...
	if(!bResult && lpConsole->bProcessInternal)
	{
		bResult=Con_InternalCommands(nCommand, szParams, hConsole);
	}
	
	//If the command was not processed we'll type out a warning.
	if(!bResult)
...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD fdwReason, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_DETACH:
		...
		Debug_printf("%u allocations left.\n", g_nBlocks);
		break;
	}
	return TRUE;
}
