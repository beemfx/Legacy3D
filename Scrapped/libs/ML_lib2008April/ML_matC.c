...
ML_MAT* ML_FUNC ML_MatRotationYawPitchRoll(ML_MAT* pOut, float Yaw, float Pitch, float Roll)
{
	...
	*/
	/*
	ML_MAT matTmpX, matTmpY, matTmpZ;
	ML_MatRotationX(&matTmpX, Pitch);
	ML_MatRotationY(&matTmpY, Yaw);
	ML_MatRotationZ(&matTmpZ, Roll);
	ML_MatMultiply(pOut, &matTmpZ, &matTmpX);
	ML_MatMultiply(pOut, pOut, &matTmpY);
	*/
	/*
	ML_MatRotationZ(pOut, Roll);
	ML_MatRotationX(&matTmp, Yaw);
	ML_MatMultiply(pOut, &matTmp, pOut);
	ML_MatRotationY(&matTmp, Pitch);
	ML_MatMultiply(pOut, &matTmp, pOut);
	*/
}
