...
int Arc_WriteZLIBData(void* uncmpdata, ARCENTRY2* entry, void* fout);
	...
	PERCENTAGEFUNC pPercentageFunc)
{
	...
	if(!lpTree)
	{
		Debug_printf("Arc_Archive: Could not generate tree!\n");
		return 0;
	}
	...
	if(nNumFiles<1)
	{
		...
		Debug_printf("Arc_Archive: No files to archive!\n");
		return 0;
	}
	...
	PERCENTAGEFUNC pPercentageFunc)
{
	...
	fout=L_fopen(szOut, LOPEN_CREATE_ALWAYS, LOPEN_WRITE);//"wb+");
	if(!fout)
		return 0;
	...
	for(entry=lpList; entry; entry=entry->m_next)
	{
		...
		fin=L_fopen(entry->m_szFilename, LOPEN_OPEN_EXISTING, LOPEN_READ);//"rb");
		if(!fin)
		{
			Debug_printf(
			...
			{
				Debug_printf("Could not write zeroes for empty file, archive is corrupt!\n");
				L_fclose(fout);
				return 0;
			}
			continue;
		...
		if(!data)
		{
			Debug_printf("Could not allocate memory for archiving \"%s\"\n", entry->m_szFilename);
			if(L_fseek(fout, entry->m_nUncmpSize, SEEK_CUR))
			{
				Debug_printf("Could not write zeroes for empty file, archive is corrupt!\n");
				L_fclose(fout);
