...
LF_FILE2 File_CreateNew(L_pstr szFilename, L_dword nSize, L_dword dwAccess)
{
	LF_FILE_S* lpNewFile=L_null;
	void* lpBuffer=L_null;
	char* szFilenameLong;
	...
	else
	{
		...
		LF_MkDir(szFilenameLong);
		fin=L_fopen(szFilenameLong, LOPEN_CREATE_ALWAYS, LOPEN_WRITE|LOPEN_READ);//"wb");
		if(!fin)
		{
...
LF_FILE2 File_OpenExisting(L_lpstr szFilename, L_dword dwAccess)
{
	...
	fin=L_fopen(szFilename, LOPEN_OPEN_EXISTING, LOPEN_READ|LOPEN_WRITE);//"rb+");
	if(!fin)
	{
...
{
	LF_FILE_S* lpNewFile=L_null;
	char szFullPathNameTemp[MAX_F_PATH];
	char* szFullPathName=szFullPathNameTemp;
	int n=0;
	...
	LF_BuildPath(szFullPathNameTemp, szFilename);
	szFullPathName=szFullPathNameTemp;
	
	
	//Eliminate the directory.
	/*
	n=L_strlen(g_szCurDir);
	if(L_strnicmp(szFullPathNameTemp, g_szCurDir, n))
	{
		switch(szFullPathName[n])
		{
		case '\\':
		case '/':
		case 0:
			szFullPathName=&szFullPathNameTemp[n+1];
			break;
		default:
			szFullPathName=szFullPathNameTemp;
			break;
	...
	*/
	...
	case LFCREATE_OPEN_EXISTING:
		return File_OpenExisting(szFullPathName, dwAccess);
	case LFCREATE_CREATE_NEW:
		return File_CreateNew(szFullPathName, dwSize, dwAccess);
	default:
		return L_null;
	}
}
...
L_bool File_Close(LF_FILE2 lpFile2)
{
	...
	else
	{
		if(L_CHECK_FLAG(lpFile->dwAccess, LF_ACCESS_WRITE))
		{
			...
			fout=L_fopen(lpFile->szFilename, LOPEN_CREATE_NEW, LOPEN_WRITE);//"wb");
			if(fout)
			{
...
L_dword File_Seek(LF_FILE2 lpFile2, L_long nMoveDist, LF_MOVETYPE nMoveType)
{
	...
	if(lpFile->dwType==TYPE_FILESTREAM)
	{
		...
		{
		case MOVETYPE_SET:
			L_fseek(lpFile->lpFile, nMoveDist, LFSEEK_SET);
			break;
		case MOVETYPE_END:
			L_fseek(lpFile->lpFile, nMoveDist, LFSEEK_END);
			break;	
		case MOVETYPE_CUR:
			L_fseek(lpFile->lpFile, nMoveDist, LFSEEK_CUR);
			break;
		}
...
/* Functions for fio access compatibility */
void* __cdecl File_fopen(const char* filename, const char* mode)
{
	//Need to impliment this, the difficult part is parsing
	//the mode options.
	
	return L_null;
}
int __cdecl File_fclose(void* stream)
{
	return File_Close(stream);
}
int __cdecl File_fseek(void* stream, long offset, int origin)
{
	return File_Seek(stream, offset, (LF_MOVETYPE) origin);
}
long __cdecl File_ftell(void* stream)
{
	return File_Tell(stream);
}
unsigned int __cdecl File_fread(void* buffer, unsigned int size, unsigned int count, void* stream)
{
