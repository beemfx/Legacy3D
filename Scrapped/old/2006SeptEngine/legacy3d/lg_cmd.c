...
int LGC_ConCommand(unsigned long nCommand, const char* szParams, HLCONSOLE hConsole, void* pExtra)
{
	...
	case CONF_TEXFORMATS:
		LV_SupportedTexFormats(
			...
			lpGame->v.m_nBackBufferFormat);

		break;
	case CONF_VIDMEM:
	{
		L_dword nMem=IDirect3DDevice9_GetAvailableTextureMem(lpGame->v.m_lpDevice);
	...
	case CONF_D3DCAPS:
	{
		...
		nResult=IDirect3DDevice9_GetDeviceCaps(lpGame->v.m_lpDevice, &d3dcaps);
		if(L_failed(nResult))
		{
